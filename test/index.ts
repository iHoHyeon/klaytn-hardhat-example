...
describe('KlayBank의 인스턴스가 생성되어야 합니다.', function () {
  before(async function () {
    klayBank = (await ethers.getContractAt('KlayBank', deployedContract)) as KlayBank;
  });
  ...
  it('계정 주소가 0인 계정으로 입금할 때 트랜잭션이 되돌려져야합니다.', async function () {
    ...
    await expect(klayBank.connect(_depositorSigner).withdraw(ethers.constants.AddressZero, '5000000000')).to.be.revertedWith(
      'KlayBank: Cannot Send to Address Zero'
    );
  });
  ...
  it('이체 금액이 잔액보다 클 때 트랜잭션이 되돌려져야합니다.', async function () {
    ...
    const balance: any = await klayBank.getBalance(_depositor1);
    await expect(klayBank.connect(_depositorSigner).withdraw(signer[1].address, balance.toString() + '1000000')).to.be.revertedWith(
      'KlayBank: Insufficient Balance'
    );
